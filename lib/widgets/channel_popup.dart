...
class ChannelPopup extends StatefulWidget {
  final bool bluetoothClassic; //if true then the user wants to connect to a bluetooth classic device and BLE if false
...
class ChannelPopupState extends State<ChannelPopup> {
  List<String> _channelNames = ["Glucose", "Lactate"]; // the list of channels (defaults to just glucose and lactate)
  bool _addingName = false; // tracks whether a name is being added or not
  final TextEditingController _controller = TextEditingController(); // Controller to add names to the list
  String? _dir;
  String? _filename;
    ...
    showDialog<String>(
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          ...
          actions: [ // Create the buttons to save the filename or cancel
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ListView.builder(
              itemCount: _channelNames.length + (_addingName ? 1 : 0), // number of items in the ListView (add 1 if adding a channel name)
              // build the ListView
                ...
                if (_addingName && index == _channelNames.length) {
                  return TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      border: OutlineInputBorder(),
                      suffixIcon: IconButton( // Create the button to submit the channel name to be added
                        onPressed: () {
                          // if the controller is not empty, add the channel name to the channel name list and to the ListView
                          if (_controller.text.isNotEmpty) {
                            setState(() {
                              _channelNames.add(_controller.text); // add the new name to the channel name list
                              _addingName = false; // No longer adding a name
                              _controller.clear(); // clear the controller for reuse
                            });
                          }
                        },
                        icon: Icon(Icons.check), // set the icon of the button to a check mark
                      ),
                    ),
                  );
                }
            ...
            Row(
              children: [
                ...
                ElevatedButton(
                  onPressed: () {
                    setState(() {
                      _addingName = true; // set the adding name parameter to true and rebuild the widget
                    });
                  },
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    List<RustStreamSink<int>> streamSinks = List.filled(numChannels, RustStreamSink<int>());

                    // Create the data handler object and add it to the app state
                    DataHandler dataHandler = DataHandler(streamSinks: streamSinks, numChannels: numChannels, dir: _dir!, filename: _filename!);
                    ...
                    if (widget.bluetoothClassic) {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => BluetoothClassicListPage()),
                      );
                    } else {
                      // Navigate to bluetooth LE device list
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => BluetoothLEDevicePage())
                      );
                    }
                  },
                  child: Text("Close"),
