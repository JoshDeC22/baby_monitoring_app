...
class GraphWidget extends StatefulWidget {
  ...
  final String paramName; // the name of the parameter that is being graphed (e.g. Glucose)
...
class GraphWidgetState extends State<GraphWidget> {
  ...
  late DateTimeAxis _xAxis; // the x axis here is created when the state is created since it will be the same for any type of plot
  late RustStreamSink<int>? dataStream; // if the plot is live this is the data source
  late DateTimeAxisController _axisController; // controller to retrieve information about the x axis
  ...
  void initState() {
    ...
    _xAxis = DateTimeAxis(
      ...
      initialVisibleMaximum: widget.data.last.time, // Initially show the last data point
    ...
    return Card(
      ...
      child: Column(
        children: [
          ...
          ListTile(
            title: Text('Graph ${widget.number} - ${widget.paramName}'), // Title of the graph
          ...
          ValueListenableBuilder<List<CartesianChartAnnotation>>(
            valueListenable: annotations, // Listen for updates to the annotations list
              ...
              return SizedBox(
                ...
                child: GestureDetector(
                  onDoubleTapDown: (details) {
                    final RenderBox box = context.findRenderObject() as RenderBox; // Get the box the chart is contained within

                    final widgetWidth = box.size.width; // find the pixel width of the chart

                    final x = details.localPosition.dx; // get the pixel position of the click

                    final dataPoint = _getDataPointFromX(x, widgetWidth, _axisController, widget.data); // get the closest data point to the click

                    _showCommentPopup(context, dataPoint, annotations); // Show the popup to enter the comment
                  },
                  // Create the SfCartesianChart
                  child: _createPlot(dataStream, widget.lineColor, widget.data, _xAxis, annotations, widget.plotType, _tooltipBehavior, _zoomPanBehavior),
                ),
              );
            },
          ),
...
class ExpandedGraphPageState extends State<ExpandedGraphPage> {
  late ZoomPanBehavior _zoomPanBehavior; // controller for zooming
  late TooltipBehavior _tooltipBehavior; // controller for interacting with the plot
  late DateTimeAxis _xAxis; // the x axis here is created when the state is created since it will be the same for any type of plot
  late DateTimeAxisController _axisController; // controller to retrieve information about the x axis
  late RustStreamSink<int>? dataStream; // if the plot is live this is the data source
  ...
  void initState() {
    ...
    _xAxis = DateTimeAxis(
      ...
      edgeLabelPlacement: EdgeLabelPlacement.shift, //prevents time labels at edges from being cut off
      initialVisibleMinimum: widget.data.last.time.subtract(const Duration(minutes: 5)), // Intially show post recent 5 minutes of data
      initialVisibleMaximum: widget.data.last.time, // Initially make the visible maximum the last data point
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Expanded Graph - ${widget.paramName}')), // Title of the graph
      body: Center(
        ...
        child: ValueListenableBuilder<List<CartesianChartAnnotation>>(
          valueListenable: widget.annotations, // Listen for updates to the annotations list
          builder: (BuildContext context, List<CartesianChartAnnotation> annotationList, Widget? child) {
            // GestureDetector detects when the plot is double clicked to add an annotation
            return GestureDetector(
              onDoubleTapDown: (details) {
                final RenderBox box = context.findRenderObject() as RenderBox; // Get the box that the plot is contained within

                final widgetWidth = box.size.width; // get the width of the chart in pixels

                final x = details.localPosition.dx; // get the x location of the double click in pixels

                final dataPoint = _getDataPointFromX(x, widgetWidth, _axisController, widget.data); // find the closest data point to the double click

               _showCommentPopup(context, dataPoint, widget.annotations); // Show the popup to enter the annotation
              },
              // Create the SfCartesianChart
              child: _createPlot(dataStream, widget.lineColor, widget.data, _xAxis, widget.annotations, widget.plotType, _tooltipBehavior, _zoomPanBehavior),
            );
          },
        ),
      ),
...
ChartData _getDataPointFromX(double x, double width, DateTimeAxisController axisController, List<ChartData> data) {
  ...
  final numPoints = xMaxIndex - xMinIndex + 1; // Find the number of data points within the visible axis

  final xProportion = x / width; // get the proportional position of the double click

  final index = xMinIndex + (numPoints * xProportion).floor(); // Find the closest index to the double click
...
// data list.
Widget _createPlot(RustStreamSink<int>? dataStream, Color lineColor, List<ChartData> data, DateTimeAxis xAxis, ValueNotifier<List<CartesianChartAnnotation>> annotations, String plotType, TooltipBehavior tooltipBehavior, ZoomPanBehavior zoomPanBehavior) {
  double animationDuration = plotType == "s" ? 0 : 0.1; // set the animation duration to 0 if the plot is static and 0.1 if it is live
  ...
  SfCartesianChart plotWidget = SfCartesianChart(
    ...
    annotations: annotations.value, // set the annotations (that can be updated dynamically)
  );
